...
{
    proto::GetSubmapDataRequest request;
    submap_id.ToProto(request.mutable_submap_id());
    async_grpc::Client<handlers::GetSubmapDataSignature> client(client_channel_);
    CHECK(client.Write(request));
    if (client.response().error_msg() != "")
    {
        // some error occurred
        return {};
    }
    ...
    {
        auto response = client.response().submap_data_query_response();
        if(!response.submap().has_submap_2d() && !response.submap().has_submap_3d())
        {
            LOG(WARNING) << "Unexpected submap data received.";
            return {};
        }
        if (response.submap().has_submap_2d())
        {
            return {std::make_shared<const mapping::Submap2D>(mapping::Submap2D(response.submap().submap_2d())), transform::ToRigid3(response.pose())};
        }
        else
        {
            return {std::make_shared<const mapping::Submap3D>(mapping::Submap3D(response.submap().submap_3d())), transform::ToRigid3(response.pose())};
        }
    }
}
